    ...
    <p:choose name="transform">
        <p:when test="$include-obfl='true'">
            ...
            <px:message cx:depends-on="parameters">
                ...
                <p:with-option name="message" select="concat('[progress px:html-to-pef.convert 76 ',(if (contains($transform,'dotify')) then 'px:dotify-transform' else 'px:transform'),'] Transforming from XML with inline CSS to OBFL')"/>
            </px:message>
            ...
            <px:message cx:depends-on="parameters">
                <!-- if $transform contains 'dotify'; use 'px:dotify-transform' as progress substep since there's currently no way to
                     send messages from java to the execution log. See: https://github.com/daisy/pipeline-issues/issues/477 -->
                <p:with-option name="message" select="concat('[progress px:html-to-pef.convert 76 ',(if (contains($transform,'dotify')) then 'px:dotify-transform' else 'px:transform'),'] Transforming from OBFL to PEF')"/>
            </px:message>
            <px:transform>
